...
def ha_forward(img, num=100):
    ...
    for i in range(num):
        if i == 0:
            ...
            Hs.append(torch.tensor(
                sample_homography_corners((w, h), patch_shape=(w, h), **H_params)[0], dtype=torch.float, device=device))
    ...
    sp_image_tensor = torch.tensor(img, dtype=torch.float32, device=device).unsqueeze(0).unsqueeze(0)
    ...
    for i in range(n_mini_batch):
        ...
        for j, img in enumerate(warped_imgs):
            with torch.no_grad():
                ...
                score = torch.tensor(warped_heatmap, dtype=torch.float32, device=device).unsqueeze(0)
                ...
                count = warp_perspective(
                    ...
                    H[j].unsqueeze(0), (h, w), mode='nearest')

                count = erosion(count, erosion_kernel)
                count = warp_perspective(count, H_inv.unsqueeze(0), (h, w), mode='nearest')[:, 0]
                score = warp_perspective(score[:, None], H_inv.unsqueeze(0), (h, w),
                                         mode='bilinear')[:, 0]
