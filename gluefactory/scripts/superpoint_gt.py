...
# import Base model from /home/egoedeke/OneDrive/ETH/3DVision/glue-factory/gluefactory/models/base_model.py
from gluefactory.models.base_model import BaseModel

# import Superpoint from gluefactory/models/extractors/superpoint_open.py
...
def ha_df(img, num=100, border_margin=3, min_counts=5):
    ...
    with torch.no_grad():
        ...
        for i in range(num):
            ...
            warpedback_kp = warp_points(keyp, homography['H_'], inverse=True)
            warpedback_kp = np.floor(warpedback_kp).astype(int)
            
            scores_xy = np.zeros((w, h), dtype=np.float32)

            for j in range(len(warpedback_kp)):
                x, y = warpedback_kp[j][0] + 1, warpedback_kp[j][1] + 1
                if x < w and y < h:
                    aggregated_heatmap[y, x, i] = scores[j]
            
            
            # for each row j in warpedback_kp add scores[j] to aggregated_heatmap
            for j in range(len(warpedback_kp)):
                x, y = warpedback_kp[j][0], warpedback_kp[j][1]
                if x < w and y < h:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--random_contrast', action='store_true', help='Add random contrast to the images (disabled by default).')
